...
import type { ExpedienteDocumento, DocumentType } from '@/lib/credit/expediente';
...
export function useExpediente() {
  ...
  const [alertas, setAlertas] = useState<ExpedienteDocumento[]>([]);
  ...
  return { documentos, alertas, isLoading, error, load, loadAlertas, add };
}
