      ...
      <main className="flex-1 p-4 md:p-8">
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div className="space-y-1">
            <h2 className="text-4xl font-headline font-bold text-primary">Catálogo de Empresas SAP</h2>
          ...
          <div className="flex gap-3">
            <Button onClick={handleOpenAssignmentDialog} variant="outline" className="rounded-xl h-12 px-6 font-bold gap-3 border-primary/20 text-primary hover:bg-primary/5">
              <Users className="w-5 h-5" /> Asignar Empresas a Usuario
            </Button>
            <Button onClick={handleCreate} className="bg-primary hover:bg-primary/90 rounded-xl h-12 px-8 font-bold gap-3 shadow-xl shadow-primary/20 text-white">
        ...
        <Card className="border-none shadow-xl rounded-2xl bg-white overflow-hidden">
          <CardHeader className="bg-muted/10 border-b">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-xl font-headline text-primary">Empresas Configuradas</CardTitle>
                <CardDescription className="font-medium">
                  {companies.length} {companies.length === 1 ? 'empresa' : 'empresas'} SAP registradas
                </CardDescription>
              </div>
              <Button 
                variant="outline" 
                    ...
                    <TableRow key={company.id} className="hover:bg-primary/5 transition-colors border-primary/5">
                      <TableCell className="py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="font-bold text-primary text-base">{company.friendlyName}</span>
                        </div>
                      </TableCell>
                      <TableCell className="font-mono text-sm text-muted-foreground">{company.companyDb}</TableCell>
          ...
          <DialogContent className="rounded-2xl max-w-lg">
            ...
            <form onSubmit={handleAssignmentSubmit} className="space-y-6">
                      ...
                      companies.filter(c => c.isActive).map((company) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold text-sm text-primary">{company.friendlyName}</p>
                              <p className="text-[10px] text-muted-foreground font-mono">{company.companyDb}</p>
                            </div>
                          </div>
                        </div>
                      ))
                ...
                {assignmentFormData.companyIds.length > 0 && (
                  <div className="flex items-center gap-2 p-3 bg-primary/5 rounded-xl border border-primary/10">
                    ...
                    <p className="text-sm font-medium text-primary">
                      {assignmentFormData.companyIds.length} {assignmentFormData.companyIds.length === 1 ? 'empresa' : 'empresas'} seleccionada{assignmentFormData.companyIds.length === 1 ? '' : 's'}
                    </p>
                  </div>
                )}
              </div>
              <DialogFooter className="gap-2">
                ...
                <Button
                  type="submit"
                  disabled={!assignmentFormData.userId || assignmentFormData.companyIds.length === 0}
                  className="bg-primary hover:bg-primary/90 rounded-xl gap-2 text-white"
                >
                  <Save className="w-4 h-4" />
                  Guardar Asignación
